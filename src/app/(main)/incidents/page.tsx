...
import { Plus, AlertTriangle, MapPin, Calendar, Clock, CheckCircle2, PlayCircle, XCircle, ArrowRight, Eye } from 'lucide-react'
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '1.5rem', fontWeight: 700 }}>Incidents & Complaints</h1>
                <Link href="/incidents/create" className="btn btn-primary" style={{ gap: '0.5rem' }}>
                    <Plus size={18} />
                    Report Incident
                </Link>
            </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid hsl(var(--border))', background: 'hsla(var(--muted), 0.3)' }}>
                                ...
                                <th style={{ padding: '0.75rem 1.5rem', textAlign: 'left', fontWeight: 600, color: 'hsl(var(--muted-foreground))' }}>Worker Involved</th>
                        ...
                        <tbody>
                            {incidents?.map((incident: any) => (
                                <tr key={incident.id} className="hover-row" style={{ borderBottom: '1px solid hsla(var(--border), 0.5)', transition: 'background 0.2s' }}>
                                    <td style={{ padding: '0.75rem 1.5rem' }}>
                                        <div style={{
                                            ...
                                            borderRadius: '99px',
                                            background: `${getStatusColor(incident.status || 'pending')}15`,
                                            color: getStatusColor(incident.status || 'pending'),
                                            fontSize: '0.75rem',
                                            fontWeight: 600,
